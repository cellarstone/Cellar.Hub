...
if [ "$IS_FLUENTD_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.fluentd:0.0.7 \
                      --replicas 1 \
    ...
    docker service create --name fluentd \
                      ...
                      --publish 24224:24224 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.fluentd:0.0.7
...
if [ "$IS_MONGO_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.mongodb:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name mongodb \
                      ...
                      --publish 27017:27017 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.mongodb:0.0.7
...
if [ "$IS_MQTT_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.mqtt:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name mqtt \
                      ...
                      --publish 1883:1883 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.mqtt:0.0.7
...
if [ "$IS_PROMETHEUS_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.prometheus:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name prometheus \
                      ...
                      --publish 9090:9090 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.prometheus:0.0.7
...
if [ "$IS_TELEGRAF_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.telegraf:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name telegraf \
                      ...
                      --with-registry-auth \
                      cellarstone/cellar.hub.telegraf:0.0.7
...
if [ "$IS_HUBCOREWEB_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.core.web:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-core-web \
                      ...
                      --log-opt tag="docker.cellar.hub.core.web" \
                      --with-registry-auth \
                      cellarstone/cellar.hub.core.web:0.0.7     
...
if [ "$IS_HUBCOREADMIN_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.core.admin:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-core-admin \
                      ...
                      --log-opt tag="docker.cellar.hub.core.admin" \
                      --with-registry-auth \
                      cellarstone/cellar.hub.core.admin:0.0.7     
...
if [ "$IS_HUBCOREAPI_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.core.api:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-core-api \
                      ...
                      --log-opt tag="docker.cellar.hub.core.api" \
                      --with-registry-auth \
                      cellarstone/cellar.hub.core.api:0.0.7     
...
if [ "$IS_HUBCORECDN_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.core.cdn:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-core-cdn \
                      ...
                      --publish 44404:44404 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.core.cdn:0.0.7     
...
if [ "$IS_HUBCOREWORKFLOW_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.core.workflow:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-core-workflow \
                      ...
                      --publish 44405:44405 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.core.workflow:0.0.7     
...
if [ "$IS_HUBCOREWEBSOCKETS_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.core.websockets:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-core-websockets \
                      ...
                      --publish 44406:44406 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.core.websockets:0.0.7     
...
if [ "$IS_HUBMODULE_OFFICEAPI_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.module.office.api:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-module-office-api \
                      ...
                      --publish 44513:44513 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.module.office.api:0.0.7
...
if [ "$IS_HUBMODULE_OFFICEMEETINGS_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.module.office.meetingrooms:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-module-office-meetingrooms \
                      ...
                      --publish 44511:44511 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.module.office.meetingrooms:0.0.7
...
if [ "$IS_HUBMODULE_OFFICERECEPTION_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.module.office.reception:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-module-office-reception \
                      ...
                      --publish 44512:44512 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.module.office.reception:0.0.7
...
if [ "$IS_HUBPROXY_EXIST" != "" ]; then
    ...
    docker service update --image cellarstone/cellar.hub.proxy:0.0.7 \
                      --replicas 1 \
                      --update-parallelism 2 \
                      --update-delay 5s \
                      --update-order start-first \
    ...
    docker service create --name cellar-hub-proxy \
                      ...
                      --publish 8080:8080 \
                      --with-registry-auth \
                      cellarstone/cellar.hub.proxy:0.0.7 \
                      --docker \
                      --docker.swarmmode \
