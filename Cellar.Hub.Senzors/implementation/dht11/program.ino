...
int val = 0;
int oldval_temperature = 0; // variable for reading the pin status
...
void createAP() {
  ...
  void reconnect() {
    ...
    //   setRandomString();
    //   String randomName = DeviceName + randString;
...
String getWifiNetworksHtml() {
	...
	if (netCount > 0) {
		for (int i = 0; i < netCount; i++) {
			...
			if (rssi <= 10) c = "4";
			if (rssi > 10 && rssi <= 20) c = "3";
			if (rssi > 20 && rssi <= 40) c = "2";
			if (rssi > 40 && rssi <= 60) c = "1";
			if (rssi > 80) c = "0";
			html += c;
			html += "\">";
...
void loop()
{
	...
	if (tep != oldval_temperature && vlh != oldval_humidity)
	{
        if (!mqttClient.connected()) {
            reconnect();
          }
        ...
        mqttClient.publish(MqttTopicName_temperature.c_str(), String(tep).c_str(), true);
        mqttClient.publish(MqttTopicName_humidity.c_str(), String(vlh).c_str(), true);

        oldval_temperature = tep;
        oldval_humidity = vlh;
	}
}
